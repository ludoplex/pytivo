...
class IBB(PlugIn):
    ...
    def IqHandler(self,conn,stanza):
        ...
        typ=stanza.getType()
        self.DEBUG(f'IqHandler called typ->{typ}', 'info')
...
"""
        ...
        self.DEBUG(
            f'StreamOpenHandler called sid->{sid} blocksize->{blocksize}', 'info'
        )
        ...
        if err: rep=Error(stanza,err)
            ...
            self.DEBUG(f"Opening stream: id {sid}, block-size {blocksize}", 'info')
            rep=Protocol('iq',stanza.getFrom(),'result',stanza.getTo(),{'id':stanza.getID()})
            self._streams[sid] = {
                'direction': f'<{str(stanza.getFrom())}',
                'block-size': blocksize,
                'fp': open(f'/tmp/xmpp_file_{sid}', 'w'),
                'seq': 0,
                'syn_id': stanza.getID(),
            }
        conn.send(rep)
    ...
    def OpenStream(self,sid,to,fp,blocksize=3000):
        ...
        syn = Protocol(
            ...
            'set',
            payload=[
                Node(f'{NS_IBB} open', {'sid': sid, 'block-size': blocksize})
            ],
        )
    ...
    def SendHandler(self,conn):
        ...
        for sid in self._streams.keys():
            ...
            if stream['direction'][:2]=='|>': cont=1
                ...
                if chunk := stream['fp'].read(stream['block-size']):
                    datanode = Node(
                        f'{NS_IBB} data',
                        {'sid': sid, 'seq': stream['seq']},
                        base64.encodestring(chunk),
                    )
                    stream['seq']+=1
                    ...
                    conn.send(
                        Protocol(
                            ...
                            stream['direction'][1:],
                            'set',
                            payload=[Node(f'{NS_IBB} close', {'sid': sid})],
                        )
                    )
...
"""
    ...
    def StreamCloseHandler(self,conn,stanza):
        ...
        self.DEBUG(f'StreamCloseHandler called sid->{sid}', 'info')
    ...
    def StreamBrokenHandler(self,conn,stanza):
        ...
        syn_id=stanza.getID()
        self.DEBUG(f'StreamBrokenHandler called syn_id->{syn_id}', 'info')
        for sid in self._streams.keys():
            stream=self._streams[sid]
    ...
    def StreamOpenReplyHandler(self,conn,stanza):
        """ Handle remote side reply about is it agree or not to receive our datastream.
            ...
            is agreed upon."""
        syn_id=stanza.getID()
        self.DEBUG(f'StreamOpenReplyHandler called syn_id->{syn_id}', 'info')
        for sid in self._streams.keys():
            stream=self._streams[sid]
            if stream['syn_id']==syn_id:
